...
private:
    ...
    void move (pnode & t, int left, int right, int shift) {
        ...
        if (shift == 0)
            return;
        int l = left + std::min(0ll, shift);
        int r = right + std::max(0ll, shift);
    ...
    void clear (pnode t) {
        if (!t)
            return;
        clear(t->l); t->l = nullptr;
        clear(t->r); t->r = nullptr;
        t = nullptr;
    }
...
public:
    ...
    bool insert (T x) {
        bool eq = false;
        pnode t = std::make_shared<node>(x, next_priority());
        stop_ = false;
    ...
    void merge_with(treap<T>& Tmp){
        pnode t = nullptr;
        merge(t, root_, Tmp.root_);
        root_ = t;
    }
};  // class treap