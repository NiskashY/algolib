...
unsigned long long myabs (unsigned long long n)
{
	return n;
}
...
bool lucas_selfridge (const T & n, T2 unused)
{
	...
	T2 dd;
	for (T2 d_myabs = 5, d_sign = 1; ; d_sign = -d_sign, ++++d_myabs)
	{
		dd = d_myabs * d_sign;
		T g = mygcd (n, d_myabs);
		if (1 < g && g < n)
...
template <class T>
void rec(int index, T current_val, std::vector<T>& result, std::vector<std::pair<T, unsigned>>& rec_vec){
    if(index == mp_size){
        result.push_back(current_val);
        return;
    }
    rec(index + 1, current_val, result, rec_vec);
    for(int i = 1; i <= rec_vec[index].second; i++){
        current_val *= rec_vec[index].first;
        rec(index+1, current_val, result, rec_vec);
    }
}

template <class T>
void get_divisors(std::map<T, unsigned> & mp, std::vector<T>& result){
    mp_size = (int)mp.size();
    std::vector<std::pair<T, unsigned>> rec_vec;
    for(auto& [k, v]:mp){
        rec_vec.push_back({k, v});
    }
